...
export function WordCard({
  ...
  return (
    <div
      className={`rounded-2xl bg-card border border-border shadow-sm p-5 space-y-2 transition-colors ${id ? "cursor-pointer hover:bg-card-hover" : ""}`}
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {phonetic && (
          <button
            ...
            className="inline-flex items-center gap-1 rounded-full bg-gray-100 px-2.5 py-1 text-xs text-muted hover:text-foreground transition-colors"
          >
            {phonetic}
        ...
        {!phonetic && (
          <button
            onClick={(e) => { e.stopPropagation(); speak(word); }}
            className="inline-flex items-center justify-center rounded-full bg-gray-100 h-7 w-7 text-muted hover:text-foreground transition-colors"
          >
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="h-3.5 w-3.5">
              <path d="M10.5 3.75a.75.75 0 00-1.264-.546L5.203 7H2.667a.75.75 0 00-.7.48A6.985 6.985 0 001.5 10c0 .887.165 1.737.468 2.52.111.29.39.48.7.48h2.535l4.033 3.796A.75.75 0 0010.5 16.25V3.75zM14.325 4.317a.75.75 0 011.061-.02 8.479 8.479 0 010 11.406.75.75 0 11-1.06-1.06 6.979 6.979 0 000-9.265.75.75 0 01-.001-1.06z" />
            </svg>
